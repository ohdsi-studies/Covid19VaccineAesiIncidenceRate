...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (724905)
...
WHERE (E.start_date >= DATEFROMPARTS(2020, 12, 1) and E.start_date <= DATEFROMPARTS(2020, 12, 31))
GROUP BY e.person_id, e.event_id
...
(
  ...
  (
    ...
    (
      --cteEndDates
      select PERSON_ID, DATEADD(day,-1 * 112,EVENT_DATE) as END_DATE -- unpad the end date by 112
      FROM
      (
				...
				(
					...
					-- add the end dates with NULL as the row number, padding the end dates by 112 to allow a grace period for overlapping ranges.
					select PERSON_ID, DATEADD(day,112,DRUG_EXPOSURE_END_DATE), 1 as EVENT_TYPE, NULL
					FROM #drugTarget D
				) RAWDATA
      ) E
