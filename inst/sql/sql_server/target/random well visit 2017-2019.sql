...
from
	(
  select t0.subject_id, t2.procedure_date as cohort_start_date, t2.procedure_date as cohort_end_date 
  from
  (
  ...
  (
    select person_id, procedure_date
  	from
  	(
  	   select po1.person_id, row_number() over (partition by po1.person_id order by newid()) as rn1, procedure_date
